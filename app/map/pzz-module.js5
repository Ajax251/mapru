...
function openPzzViewer(pzzData) {
    ...
    const stopProp = (e) => e.stopPropagation();
    overlayEl.addEventListener('paste', stopProp);
    overlayEl.addEventListener('keydown', stopProp);
    overlayEl.addEventListener('keyup', stopProp);
    overlayEl.addEventListener('keypress', stopProp);
    ...
    const zoneSearchInput = document.getElementById('viewer-zone-search');
    const useSearchInput = document.getElementById('viewer-use-search');

    zoneSearchInput.addEventListener('input', e => {
        zoneTerms = e.target.value.toLowerCase().split(/\s+/).filter(t => t.length > 0);
        updateView();
    });
    zoneSearchInput.addEventListener('keydown', stopProp);

    useSearchInput.addEventListener('input', e => {
        useTerms = e.target.value.toLowerCase().split(/\s+/).filter(t => t.length > 0);
        updateView();
    });
    useSearchInput.addEventListener('keydown', stopProp);
