    ...
    meta.height = (meta.floorsNumeric * 3.5) * mercatorScale;
            ...
            allLocalFeatures.buildings.forEach(b => {
                if ((b.type === 'Point' || !b.meta.isSpatial) && b.meta.rawArea) {
                    ...
                    // 1. Ищем родительский земельный участок
                    ...
                    // 2. Если по точке не нашли, берём первый целевой участок
                    if (!parentPoly && allLocalFeatures.parcels.length > 0) {
                         parentPoly = allLocalFeatures.parcels[0].polygons[0][0];
                    }

                    // 3. Устанавливаем точку в центр участка
                    if (!pt && parentPoly) {
                        pt = getBBoxCenter(parentPoly);
                    }
                    ...
                    // Определяем угол поворота по самой длинной стороне ЗУ
                    let angle = 0;
                    if (parentPoly) {
                        let maxLenSq = 0;
                    ...
                    // Выбираем пропорции прямоугольника (Длина к Ширине)
                    let ratio = 1.2; // Частный дом (почти квадрат)
                    if (b.meta.rawText.includes('многоквартир') || b.meta.rawText.includes('мкд') || b.meta.rawText.includes('школ')) {
                        ratio = 2.5; // Вытянутый МКД
                    } else if (b.meta.rawText.includes('гараж') || b.meta.rawText.includes('склад') || b.meta.rawText.includes('цех')) {
                        ratio = 2.0; 
                    }

                    // Считаем истинные метры
                    let trueWidth = Math.sqrt(footprintArea / ratio);
                    let trueLength = trueWidth * ratio;

                    // Умножаем на масштаб проекции, чтобы 10м в цифрах выглядели как 10м на карте
                    let scaledW = trueWidth * mercatorScale;
                    let scaledL = trueLength * mercatorScale;

                    let halfW = scaledW / 2;
                    let halfL = scaledL / 2;

                    // Вытягиваем дом вдоль оси X локальных координат (которая будет повернута вдоль забора)
                    let rect =[
