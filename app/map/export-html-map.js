...
<body>
  ...
  <!-- Модальное окно выносок -->
    ...
    ymaps.layer.storage.add('osm#map', function () {
                ...
                return new ymaps.Layer('https://basemaps.cartocdn.com/rastertiles/voyager/%z/%x/%y.png', {
                    projection: ymaps.projection.sphericalMercator
                });
            });
                ...
                '<div style="position: absolute; pointer-events: none;">' +
                    ...
                    '<div style="position: absolute; left: $[properties.textLeft]px; top: $[properties.textTop]px; transform: translateY(-100%); background: #fff; border: 1.5px solid #000; padding: 3px 6px; color: #000; white-space: nowrap; box-shadow: 2px 2px 5px rgba(0,0,0,0.3); line-height: 1; box-sizing: border-box;">' +
                        '{% if properties.isInline %}' +
                            '<div>' +
                                '<span style="font-family: Arial; font-size: $[properties.cnSize]px; font-weight: $[properties.cnWeight]; margin-right: 5px;">$[properties.cn]</span>' +
                                '<span style="font-family: Arial; font-size: $[properties.addrSize]px; font-weight: $[properties.addrWeight];">$[properties.addr]</span>' +
                            '</div>' +
                        '{% else %}' +
           ...
           function clearSelection() {
            ...
            // --- НАЧАЛО: Логика выбора наименьшего объекта при наложении ---
            function getApproxArea(geometry) {
            ...
            function getSmallestPolygonAt(coords, defaultTarget) {
                if (defaultTarget && defaultTarget.geometry && defaultTarget.geometry.getType() !== 'Polygon') return defaultTarget;
                var clickedObjects = [];
                ['ZU', 'OKS', 'Structure', 'ZOUIT'].forEach(function(layerName) {
                    if (!activeLayers[layerName]) return;
                    layerGroups[layerName].forEach(function(obj) {
                        if (obj.options.get('visible') !== false && obj.geometry.getType() === 'Polygon') {
                            // Проверяем, попадают ли координаты клика в этот полигон
                            if (obj.geometry.contains(coords)) {
                ...
                if (clickedObjects.length > 0) {
                    ...
                    clickedObjects.sort(function(a, b) {
                        return getApproxArea(a.geometry) - getApproxArea(b.geometry);
                    });
                    return clickedObjects[0];
                }
                return defaultTarget;
            }
            // --- КОНЕЦ: Логика выбора наименьшего объекта ---
            ...
            mapData.forEach(function(item) {
                ...
                if (item.type === 'Polygon' || item.type === 'LineString') {
                    ...
                    geoObject.events.add('click', function (e) {
                        ...
                        var originalTarget = e.get('target');
                        
                        // Ищем самый маленький полигон под местом клика
                        var target = getSmallestPolygonAt(clickCoords, originalTarget);
                ...
                layerGroups['ZU'].forEach(function(poly) {
                    ...
                    if (!isTarget) {
                      ...
                      var finalAddr = "";
                        ...
                        // Определяем, объединяем ли мы текст в одну строку
                        var isInline = (showCadNum && shortCadNum && showAddress && finalAddr !== "");
                        ...
                        if (isInline) {
                            ...
                            inlineWidth += ctx.measureText(cadNum).width;
                            inlineHeight = Math.max(inlineHeight, cnSize);
                            
                            ctx.font = addrWeight + " " + addrSize + "px Arial";
                            inlineWidth += ctx.measureText(" " + finalAddr).width + 5; // +5px margin-right
                            inlineHeight = Math.max(inlineHeight, addrSize);

                            boxWidth += Math.ceil(inlineWidth);
                            boxHeight += inlineHeight;
                        } else {
                            ...
                            if (showAddress && finalAddr) {
                                ...
                                boxHeight += addrSize;
                                if (showCadNum) boxHeight += 4; // margin-top
                            }
