    ...
    setTimeout(() => {
        try {
            ...
            const analyzeFeature = (f, category) => {
                ...
                // Приоритет имени для разных типов
                ...
                if (meta.area && !isNaN(parseFloat(meta.area))) {
                    ...
                    meta.area = o.params_extension + ' м (длина)';
                     ...
                     // Для ЗОУИТ добавим ограничение в поле area для отображения
            ...
            // Цвета модального окна (адаптивные к теме не нужны тут, это контейнер)
            ...
            // Стили заголовка зададим динамически при создании iframe, или тут инлайн для старта
            ...
            // Кнопка темы (будет отправлять сообщение в iframe)
            const themeBtn = createWinBtn(savedTheme === 'dark' ? 'fas fa-sun' : 'fas fa-moon', '#f59e0b', () => {
                ...
                // Обновляем стили самого модального окна
...
<style>
    ...
    #close-ui-btn {
        display: none; position: absolute; top: 15px; right: 15px; background: none; border: none; font-size: 26px; color: var(--text-muted); cursor: pointer; line-height: 1; padding: 0;
    }
    ...
    #ui-panel.hidden { display: none; } /* Для десктопа если нужно скрыть */
...
let scene, gridHelper, dirLight, ambientLight;
...
try {
    ...
    // Освещение (начальное, будет обновлено updateSceneTheme)
    ...
    function createGroundTexture(){
        ...
        ctx.fillStyle="#808080";ctx.fillRect(0,0,512,512); // Base
        ctx.fillStyle="#858585";ctx.fillRect(0,0,256,256);ctx.fillRect(256,256,256,256);
    ...
    const groundMat = new THREE.MeshStandardMaterial({map:createGroundTexture(), roughness:1.0, color: 0xffffff});
    const ground = new THREE.Mesh(new THREE.PlaneGeometry(2000,2000), groundMat);
    ground.rotation.x = -Math.PI/2; ground.receiveShadow = true; scene.add(ground);
    
    // Сетка
    gridHelper = new THREE.GridHelper(2000, 100, 0x000000, 0x000000);
    ...
    window.updateSceneTheme = function() {
        if(currentTheme === 'dark') {
            ...
            gridHelper.material.color.setHex(0x334155); // Center
            // GridHelper colors are tricky to update after creation, recreate or accept defaults
            // Простой способ поменять цвета сетки - через её geometry colors (сложно) или пересоздать
            // Для упрощения меняем только opacity/color материала если возможно, или фон
            ground.material.color.setHex(0x1e293b); // Dark ground tint
        } else {
            scene.background = new THREE.Color(0xe0f2fe);
            scene.fog = new THREE.FogExp2(0xe0f2fe, 0.0015);
            ambientLight.intensity = 0.7;
            dirLight.intensity = 2.0;
            ground.material.color.setHex(0xffffff); // Light ground tint
        }
    }
    // Вызываем сразу для старта
    updateSceneTheme();
    ...
    const attachMeta = function(mesh, meta, category) {
        if (!mesh) return; // Защита от undefined
    ...
    data.zouits.forEach(function(z){
        ...
        z.polygons.forEach(function(poly){
            ...
            if(z.type==="Line"){
                ...
                attachMeta(zone, z.meta, getZouitShort(z.meta)); // <-- FIX: Added hints for ZOUIT lines
                midPt=pts[Math.floor(pts.length/2)];
            } else {
                var shape=createShape(poly);
                if(shape.getPoints().length>2){
                    ...
                    attachMeta(mesh, z.meta, getZouitShort(z.meta)); // <-- FIX: Added hints for ZOUIT polygons
                    var c=getCentroid(poly[0]);
