    ...
    modal.innerHTML = `
        <div class="color-modal-content" style="padding: 20px; text-align: left;">
            ...
            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 20px;">
                ...
                <label for="sch_skipLoad" style="font-size: 0.95em; cursor: pointer; color: #d32f2f; font-weight: bold;">Не делать повторные запросы (быстро)</label>
            </div>
...
async function executeSchemaGeneration(lat, lon, targetPolygon, config) {
    ...
    try {
        ...
        targetPolygon.options.set({
            ...
            fillColor: `${config.fillColor}${fillHex}`,
            zIndex: 1000
        });
        ...
        if (!config.skipLoad) {
            ...
            // Пытаемся найти терр.зону среди уже загруженных меток на карте
...
async function fetchTerrZoneName(centerGeo) {
    ...
    if (res && res.features && res.features.length > 0) {
        ...
        const tzLabel = new ymaps.Placemark([centerGeo[0] + 0.0002, centerGeo[1]], { iconContent: `ТЗ: ${name}` }, { preset: 'islands#blueStretchyIcon', zIndex: 1200 });
