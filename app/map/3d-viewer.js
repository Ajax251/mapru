                            ...
                            f.polygons.forEach((polyRings) => {
                                try {
                                    ...
                                    if (turfRings.length > 0) {
                                        // Принудительно делаем rewind, чтобы контуры были валидны для Three.js
                    ...
                    objectsToCheck.forEach(obj => {
                        ...
                        try {
                            ...
                            if (intersection && intersection.geometry) {
                                ...
                                // Делаем rewind для геометрии пересечения, чтобы 3D-движок смог ее залить
...
scene = new THREE.Scene();
  ...
  controls.mouseButtons = { LEFT: THREE.MOUSE.PAN, MIDDLE: THREE.MOUSE.DOLLY, RIGHT: THREE.MOUSE.ROTATE };
    ...
    function createGroundTexture(type, hexColor){
      ...
      // Увеличиваем плотность текстуры пропорционально новому размеру земли
...
const altValueEl = document.getElementById("alt-value");
    ...
    // Скрываем лоадер сразу перед началом анимации
