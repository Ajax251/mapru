...
window.open3DVisualization = function () {
    setTimeout(() => {
      ...
      // Чтение сохраненных настроек
            ...
            const isGoogleLayerActive = ['gsat', 'ghyb', 'osm'].includes(savedGroundTex);
            ...
            const cleanAddress = (rawAddress) => {
               ...
               const to3857 = (yandexCoord) => {
                ...
                // 1. Возвращаем к "истинным" WGS84, убирая смещение Яндекса
                let trueLat = yandexCoord[0] + (window.mapOffsetY * 0.000008983);
                let trueLon = yandexCoord[1] + (window.mapOffsetX * 0.000008983);
                
                // 2. Если выбран слой Google/OSM, ДОПОЛНИТЕЛЬНО применяем KML-смещение
                if (isGoogleLayerActive) {
                    trueLat += (kmlOffsetY * 0.000008983);
                    trueLon += (kmlOffsetX * 0.000008983);
                }
                
                // 3. Проецируем финальные, скорректированные координаты
