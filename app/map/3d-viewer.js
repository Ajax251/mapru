...
try {
    ...
    const getNetShort = function(m) { if (m.isGas) return "Газ"; if (m.isWater) return "Вода"; if (m.isHeat) return "Тепло"; if (m.isElectric) return "ЛЭП"; if (m.isSewer) return "Канализация"; return "Сеть"; };
    const getZouitShort = function(m) { if (m.isGas) return "Охранная зона Газ"; if (m.isWater) return "Охранная зона Вода"; if (m.isHeat) return "Охранная зона Тепло"; if (m.isElectric) return "Охранная зона  ЛЭП"; if (m.isSewer) return "Охранная зона Канализация"; return "ЗОУИТ"; };

    // Цвета ЗОУИТ аналогично основной карте
   ...
   const getInfraColor = function(m) { if (m.isGas) return INFRA_COLORS.gas; if (m.isWater) return INFRA_COLORS.water; if (m.isHeat) return INFRA_COLORS.heat; if (m.isElectric) return INFRA_COLORS.electric; if (m.isSewer) return INFRA_COLORS.sewer; return INFRA_COLORS.def; };
    ...
    function flyToMesh(targetObj) {
        ...
        box.getSize(size);
        let maxDim = Math.max(size.x, size.y, size.z, 20); 
        
        let endCam = center.clone().add(dir.multiplyScalar(maxDim * 2.5)); 
        endCam.y = Math.max(endCam.y, maxDim);
    ...
    const interactables =[];
    // Улучшенный attachMeta для целых групп
        ...
        t.polygons.forEach(function(poly){
            ...
            if(t.type==="Line"){
                ...
                if(shape.getPoints().length>2){
                    ...
                    addLabel(c, 10, "Цель", "", t.meta, "#ef4444", tGrp);
                }
            }
        });
    });
...
data.intersections.forEach(function(iObj){
        iObj.polygons.forEach(function(poly){
            ...
            if(shape.getPoints().length>2){
                ...
                attachMeta(mesh, iObj.meta, "Наложение"); 
        ...
        if(b.meta.isSpatial){
            b.polygons.forEach(function(poly){
                ...
                if(shape.getPoints().length>2){
                    ...
                    attachMeta(bModel, b.meta, "ОКС (Здание)");
    ...
    data.zouits.forEach(function(z){
     ...
     if(z.type==="Line"){
                ...
                if(shape.getPoints().length>2){
                    ...
                    mesh.rotation.x=-Math.PI/2;mesh.position.y=0;zGrp.add(mesh);
        ...
        if (hasUniqueItems) {
            Object.keys(uniqueItems).forEach(function(id) {
               ...
               cb.addEventListener("change", function(e) {
                    ...
                    // ИЗМЕНЕНИЕ 1: Гарантируем, что главная группа видима, если включен хотя бы 1 элемент
                ...
                subLabel.addEventListener("click", function() {
                    ...
                    if (targetMesh) flyToMesh(targetMesh);
                });
            });
            ...
            // ИЗМЕНЕНИЕ 2: Синхронизируем чекбоксы
            itemCheckboxes.forEach(function(cb) { cb.checked = isVisible; });
            
            // ИЗМЕНЕНИЕ 3: Переключаем видимость группы
            groupRef.visible = isVisible; 
            
            // ИЗМЕНЕНИЕ 4: Принудительно синхронизируем видимость всех дочерних 3D-объектов!
            groupRef.children.forEach(function(child) {
                child.visible = isVisible;
            });
    ...
    const setHighlight = (obj, isHover) => {
        obj.traverse(function(child) {
            if ((child.isMesh || child.isLine || child.isLineSegments) && child.userData.origEmissive !== undefined) {
                const highlightColor = currentTheme === 'dark' ? 0x64748b : 0xbababa;
                if (Array.isArray(child.material)) {
                    child.material.forEach((m, i) => {
                        if (m.emissive) m.emissive.setHex(isHover ? highlightColor : child.userData.origEmissive[i]);
                    });
                } else if (child.material && child.material.emissive) {
                    child.material.emissive.setHex(isHover ? highlightColor : child.userData.origEmissive);
                }
            }
        });
    };
