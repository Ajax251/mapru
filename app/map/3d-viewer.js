...
const mercatorScale = 1 / Math.cos(mapLatRad);
    ...
    meta.height = (meta.floorsNumeric * 3.5) * mercatorScale;
            ...
            let titleName = obj.properties.get('cadastralNumber') || 'Целевой объект';
                ...
                let calcArea = 0;
                try {
                    // Используем вашу встроенную функцию для точного расчета площади в МСК
       ...
       // --- НАЧАЛО ИСПРАВЛЕННОЙ ЛОГИКИ НАЛОЖЕНИЙ В 3D ---
            if (window.turf && allLocalFeatures.target && allLocalFeatures.target.length > 0) {
                            ...
                            f.polygons.forEach((polyRings) => {
                                try {
                                    ...
                                    if (turfRings.length > 0) {
                                        result.push({ 
                                            poly: window.turf.polygon(turfRings), 
                ...
                targetTurf.forEach(target => {
                    ...
                    objectsToCheck.forEach(obj => {
                        // 1. Исключаем прямое совпадение ID или имен
                        ...
                        try {
                            ...
                            if (intersection && intersection.geometry) {
                                ...
                                // 2. Умная защита: Если площади объекта и пересечения совпадают на 99%,
                                // значит целевой объект и есть этот участок. Игнорируем самоналожение.
                                if (Math.abs(intersectionArea - objArea) < 0.5 && Math.abs(intersectionArea - targetArea) < 0.5) {
                                    return; 
                                }

                                let geoms = [];
                                if (intersection.geometry.type === 'Polygon') geoms = [intersection.geometry.coordinates];
                                else if (intersection.geometry.type === 'MultiPolygon') geoms = intersection.geometry.coordinates;

                                geoms.forEach(poly => {
                                    if (poly[0] && poly[0].length >= 4) {
                                        // Вычисляем плоскую площадь, чтобы отсечь микро-касания смежных границ
                                        let planarArea = 0;
                                        const pts = poly[0];
                                        ...
                                        // Порог 0.1 кв.м (защита от "шума" на смежных границах)
                                        if (planarArea > 0.1) { 
...
function createGroundTexture(type, hexColor){
  ...
  let currentGroundColor = "${savedGroundColor}";
   ...
   /* ── КОМПАС (УЛУЧШЕННЫЙ) ── */
    function createCompass(){
        ...
        // 1. Внешнее металлическое кольцо (ободок)
        ...
        // 2. Внутренний циферблат (светлая подложка)
        ...
        // 3. Центральный штифт (золотая/латунная ось)
        ...
        const addL = (text, rotY, color) => {
            ...
            // Размещаем метки аккуратно на белом циферблате внутри кольца
  ...
  function enterIsolationMode(targetObj) {
        ...
        // --- ИСПРАВЛЕНИЕ ---
        // Ищем родительские объекты (участок и целевой контур) во всех нужных слоях
        ['target', 'parcels', 'buildings'].forEach(groupName => {
            sceneGroups[groupName].children.forEach(pGrp => {
                if (pGrp.userData && pGrp.userData.meta) {
                    ...
                    // Проверяем совпадение как по ID (для ЗУ), так и по Имени (для произвольных контуров)
        ...
        labelsData.forEach(l => {
            if (l.groupData) {
                ...
                // Отображаем метки для обоих объектов + саму метку наложения
