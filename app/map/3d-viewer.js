        ...
        try {
            ...
            const analyzeFeature = (f, category) => {
                ...
                let meta = {
                    ...
                    isProcedural: false, // Флаг для искусственно созданных ОКС
            ...
            const processFeatureArray = (featuresArray, type) => {
                ...
                (featuresArray ||[]).forEach(f => {
                    ...
                    if (f.geometry.type === 'Polygon') ringsList =[f.geometry.coordinates];
                        ...
                        ringsList = [[[f.geometry.coordinates]]]; // Поддержка точек для процедурной генерации
                    }
            ...
            const getCentroidEditor = function(pts) {
                let cx = 0, cy = 0;
                pts.forEach(p => { cx += p.x; cy += p.y; });
                return { x: cx / pts.length, y: cy / pts.length };
            };

            // --- ПРОЦЕДУРНАЯ ГЕНЕРАЦИЯ УСЛОВНЫХ ОКС ---
            allLocalFeatures.buildings.forEach(b => {
                // Если это точка или объект без геометрии, но у него известна площадь
                if ((b.type === 'Point' || !b.meta.isSpatial) && b.meta.area) {
                    let footprintArea = parseFloat(b.meta.area) / (b.meta.floorsNumeric || 1);
                    if (isNaN(footprintArea) || footprintArea <= 5) return; // Минимум 5 кв.м.

                    let pt = null;
                    let parentPoly = null;

                    // Если есть координата точки - берем её
                    if (b.type === 'Point' && b.polygons[0] && b.polygons[0][0] && b.polygons[0][0][0]) {
                        pt = b.polygons[0][0][0]; 
                    }

                    // 1. Ищем родительский земельный участок по пересечению или совпадению номера
                    allLocalFeatures.parcels.forEach(parcel => {
                        if (!parcel.polygons[0] || !parcel.polygons[0][0]) return;
                        // Если точка лежит внутри участка
                        if (pt && isPointInPolyEditor(pt, parcel.polygons[0][0])) {
                            parentPoly = parcel.polygons[0][0];
                        }
                    });

                    // 2. Если по точке не нашли, но знаем КН участка (из номера ОКС)
                    if (!parentPoly) {
                        let bCnMatch = b.meta.id.match(/^(\d{2}:\d{2}:\d{6,7}:\d+)/);
                        if (bCnMatch) {
                            let baseQuarter = b.meta.id.split(':').slice(0,3).join(':'); // Квартал
                            // Берем первый попавшийся участок (на практике он обычно один целевой)
                            if (allLocalFeatures.parcels.length > 0) {
                                parentPoly = allLocalFeatures.parcels[0].polygons[0][0];
                            }
                        }
                    }

                    // 3. Если точку не нашли в геометрии API, берем центр найденного участка
                    if (!pt && parentPoly) {
                        pt = getCentroidEditor(parentPoly);
                    }

                    if (!pt) return; // Если не к чему привязаться - сдаемся

                    b.meta.isProcedural = true;
                    b.meta.isSpatial = true; 

                    // Определяем угол поворота (параллельно самому длинному забору участка)
                    let angle = 0;
                    if (parentPoly) {
                        let maxLenSq = 0;
                    ...
                    // Формируем квадрат площади
                    let half = Math.sqrt(footprintArea) / 2;
                    let square =[
                    ...
                    let rotatedPoly = square.map(p => ({
                        x: pt.x + (p.x * cosA - p.y * sinA),
                        y: pt.y + (p.x * sinA + p.y * cosA)
                    }));
                    rotatedPoly.push(rotatedPoly[0]); // замыкаем
...
function createGroundTexture(type, hexColor){
  ...
  let currentGroundColor = "${savedGroundColor}";
    ...
    const createBuildingModel=function(shape,height,style,isMini, isProcedural = false){
        ...
        if (isProcedural) {
            // Прозрачный макет для сгенерированных ОКС
    ...
    var addLayerUi = function(name, color, groupRef, dataArray) {
        ...
        if (name === "Здания (ОКС)") {
            ...
            toggleProcBtn.innerHTML = '<i class="fas fa-ghost"></i>';
        ...
        if (hasUniqueItems) {
            Object.keys(uniqueItems).forEach(function(id) {
                ...
                } else if (itemMeta.isSpatial && name !== "Земельные участки" && name !== "Наложения" && name !== "Целевой объект") {
