...
async function executeSchemaGeneration(lat, lon, targetPolygon, config) {
    ...
    try {
        ...
        if (config.autoSort) rawCoords = sortPointsClockwiseNW(rawCoords);
        ...
        // ПЕРЕДАЕМ ТЕКСТЫ НАПРЯМУЮ В ФУНКЦИЮ СКРИНШОТА
...
function processAndDrawSchemaPoints(coords, polygon, config) {
    ...
    for (let i = 0; i < coords.length; i++) {
        ...
        mskCoordsTable.push({ point: `н${i + 1}`, x: x_msk.toFixed(2), y: y_msk.toFixed(2) });
