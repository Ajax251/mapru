        ...
        try {
            ...
            const analyzeFeature = (f, category) => {
                ...
                if (category === 'building') {
                    ...
                    meta.isElectric = text.includes('электр') || text.includes('лэп') || text.includes('кабель');
...
<body>
    ...
    <script type="module">
        ...
        // Генерация Трубы (для сетей)
        const buildPipe = (pointsArray, radius, color, isUnderground) => {
            const group = new THREE.Group();
            const yOffset = isUnderground ? -2 : 3; // Под землей или на опорах
            
            const vecPoints = pointsArray.map(p => new THREE.Vector3(p.x, yOffset, -p.y));
            const path = new THREE.CatmullRomCurve3(vecPoints, false, 'catmullrom', 0.05); // Небольшое сглаживание
            const tube = new THREE.Mesh(
                new THREE.TubeGeometry(path, Math.max(20, vecPoints.length * 2), radius, 8, false), 
        ...
        data.structures.forEach(s => {
            ...
            s.polygons.forEach(poly => {
                // Берем внешний контур как линию
                const pipe = buildPipe(poly[0], radius, color, s.meta.isUnderground);
                groups.structures.add(pipe);
            });
        });

        // 5. ЗОУИТ (Объемные прозрачные зоны)
