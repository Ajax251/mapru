...
.info-text { position: absolute; bottom: 20px; left: 20px; background: rgba(255,255,255,0.95); color: #333; padding: 15px; border-radius: 8px; font-size: 13px; font-weight: 600; pointer-events: none; box-shadow: 0 4px 15px rgba(0,0,0,0.15); border-left: 4px solid #3b82f6; z-index: 100;}
...
try {
    ...
    function getWindowMaterial(style) {
        ...
        const startX = (256 - winW) / 2;
        const startY = (256 - winH) / 2;

        ctx.fillStyle = '#' + new THREE.Color(style.win).getHexString();
        ctx.fillRect(startX, startY, winW, winH);
        
        // Окно без решеток (сплошной стеклопакет)
        ctx.strokeStyle = '#1e293b'; 
        ctx.lineWidth = 8;
        ctx.strokeRect(startX, startY, winW, winH);
    ...
    // Основание (Пол в клеточку)
    function createGroundTexture() {
        const canvas = document.createElement('canvas');
        canvas.width = 512; canvas.height = 512;
        const ctx = canvas.getContext('2d');
        ctx.fillStyle = '#f1f5f9'; ctx.fillRect(0, 0, 512, 512);
        ctx.fillStyle = '#e2e8f0'; ctx.fillRect(0, 0, 256, 256); ctx.fillRect(256, 256, 256, 256);
        const tex = new THREE.CanvasTexture(canvas);
        tex.colorSpace = THREE.SRGBColorSpace; 
        tex.wrapS = THREE.RepeatWrapping; tex.wrapT = THREE.RepeatWrapping;
    ...
    // Таблички фиксированного размера (без искажений и лишних анимаций)
    ...
    const createBuildingModel = (shape, height, style, isMiniature = false) => {
        ...
        const wallMat = isMiniature ? new THREE.MeshStandardMaterial({ color: style.wall }) : getWindowMaterial(style);
        ...
        if (style.hippedRoof) { 
            ...
            const roofBaseDim = Math.sqrt(w*w + d*d) * 0.72;
    ...
    data.parcels.forEach((p, index) => {
        ...
        const parcelColor = new THREE.Color(0x7cb342); // Светло-зеленый (непрозрачный)

        p.polygons.forEach(poly => {
            const shape = createShape(poly);
            if(shape.getPoints().length > 2) {
                const mat = new THREE.MeshStandardMaterial({ color: parcelColor, roughness: 1.0 });
                ...
                const edges = new THREE.LineSegments(new THREE.EdgesGeometry(mesh.geometry), new THREE.LineBasicMaterial({ color: 0x4a822b, linewidth: 2 }));
...
const exportBtnEl = document.getElementById('export-html-btn');
    ...
    function animate() {
        ...
        // Анимация плавающих объектов (непространственные ОКС)
