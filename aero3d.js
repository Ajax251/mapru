...
export class PilotSimulator {
    constructor(containerId) {
        ...
        this.flareCanvas = document.createElement('canvas');
        this.flareCanvas.style.position = 'absolute';
        this.flareCanvas.style.top = '0';
        this.flareCanvas.style.left = '0';
        this.flareCanvas.style.width = '100%';
        this.flareCanvas.style.height = '100%';
        this.flareCanvas.style.zIndex = '1';
        this.flareCanvas.style.pointerEvents = 'none';
        this.container.appendChild(this.flareCanvas);
        this.flareCtx = this.flareCanvas.getContext('2d');
        ...
        const moonUrl = 'https://raw.githubusercontent.com/mrdoob/three.js/master/examples/textures/planets/moon_1024.jpg';
        const tMoon = tl.load(moonUrl);
    ...
    _initComposer() {
        ...
        this.bloomPass = new UnrealBloomPass(
            new THREE.Vector2(isM ? w * 0.5 : w, isM ? h * 0.5 : h),
            0.45, 0.7, 0.85
        );
        ...
        const skyMat = new THREE.ShaderMaterial({
            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    float h = vd.y;
                    float hMix = smoothstep(-0.35, 0.7, h);
                    vec3 sc = mix(bottomColor, midColor, hMix);
                    sc = mix(sc, topColor, smoothstep(0.02, 0.95, max(h, 0.0)));
                    float sDot = max(0.0, dot(vd, sd));
                    float horizonGlow = pow(1.0 - min(1.0, abs(h) * 1.7), 4.0);
                    sc += vec3(0.28, 0.46, 0.8) * horizonGlow * 0.18;
                    sc += vec3(1.0, 0.45, 0.1) * pow(sDot, 64.0) * 0.7 * sunsetIntensity;
                    sc += vec3(1.0, 0.72, 0.36) * pow(sDot, 16.0) * (0.18 + sunsetIntensity * 0.1);
                    sc += vec3(1.0, 0.94, 0.8) * pow(sDot, 700.0) * 2.6;
    ...
    _drawFlareDisc(x, y, radius, color, alpha) {
        const g = this.flareCtx.createRadialGradient(x, y, 0, x, y, radius);
        g.addColorStop(0, color.replace('__A__', alpha.toFixed(3)));
        g.addColorStop(0.45, color.replace('__A__', (alpha * 0.35).toFixed(3)));
        g.addColorStop(1, color.replace('__A__', '0'));
        this.flareCtx.fillStyle = g;
        this.flareCtx.beginPath();
        this.flareCtx.arc(x, y, radius, 0, Math.PI * 2);
        this.flareCtx.fill();
    }

    _updateSunFlare() {
        if (!this.flareCtx) return;
        const w = this.flareCanvas.width;
        const h = this.flareCanvas.height;
        this.flareCtx.clearRect(0, 0, w, h);

        const sunScreen = this.sunGlow.position.clone().project(this.camera);
        if (sunScreen.z <= -1 || sunScreen.z >= 1) return;

        const x = (sunScreen.x * 0.5 + 0.5) * w;
        const y = (-sunScreen.y * 0.5 + 0.5) * h;
        if (x < -w * 0.2 || x > w * 1.2 || y < -h * 0.2 || y > h * 1.2) return;

        const dx = x - w / 2;
        const dy = y - h / 2;
        const centerDist = Math.sqrt(dx * dx + dy * dy) / Math.min(w, h);
        const intensity = Math.max(0, 1 - centerDist * 1.35);
        if (intensity <= 0.02) return;

        this.flareCtx.globalCompositeOperation = 'screen';
        this._drawFlareDisc(x, y, 110 + intensity * 90, 'rgba(255, 245, 220, __A__)', 0.22 + intensity * 0.25);
        this._drawFlareDisc(x, y, 42 + intensity * 30, 'rgba(255, 180, 80, __A__)', 0.18 + intensity * 0.12);

        const ghosts = [
            { t: -0.18, r: 26, c: 'rgba(120, 210, 255, __A__)' },
            { t: 0.22, r: 18, c: 'rgba(255, 130, 90, __A__)' },
            { t: 0.42, r: 34, c: 'rgba(100, 255, 170, __A__)' },
            { t: 0.72, r: 22, c: 'rgba(255, 100, 180, __A__)' },
            { t: 1.08, r: 54, c: 'rgba(80, 130, 255, __A__)' }
        ];
        for (const ghost of ghosts) {
            const gx = x - dx * ghost.t;
            const gy = y - dy * ghost.t;
            this._drawFlareDisc(gx, gy, ghost.r + intensity * 18, ghost.c, 0.06 + intensity * 0.12);
        }

        this.flareCtx.strokeStyle = `rgba(255, 220, 160, ${0.06 + intensity * 0.16})`;
        this.flareCtx.lineWidth = 2;
        this.flareCtx.beginPath();
        this.flareCtx.arc(x, y, 78 + intensity * 24, 0, Math.PI * 2);
        this.flareCtx.stroke();
        this.flareCtx.globalCompositeOperation = 'source-over';
    }
    ...
    _createClouds() {
        ...
        for (let i = 0; i < 28; i++) {
            ...
            const d = 3000 + Math.random() * 20000;
    ...
    _initWeather3D() {
        ...
        this.weatherMat3D = new THREE.PointsMaterial({
            color: 0xffffff,
            size: 4,
            transparent: true,
            opacity: 0.0
    ...
    _setupControls() {
        ...
        window.addEventListener('mousemove', (e) => {
            ...
            this.lookYaw -= (e.clientX - lx) * 0.0025;
            this.lookPitch -= (e.clientY - ly) * 0.0025;
            lx = e.clientX;
            ly = e.clientY;
            this.lookPitch = Math.max(-1, Math.min(1, this.lookPitch));
        });
        this.container.addEventListener('wheel', (e) => {
            if (!this.isActive) return;
            this.mapZoom = Math.max(10, Math.min(15, this.mapZoom + (e.deltaY > 0 ? -1 : 1)));
            this.clearTiles();
        });
        ...
        this.container.addEventListener('touchmove', (e) => {
            if (!this.isActive) return;
            this.lookYaw -= (e.touches[0].clientX - tx) * 0.003;
            this.lookPitch -= (e.touches[0].clientY - ty) * 0.003;
            tx = e.touches[0].clientX;
            ty = e.touches[0].clientY;
            this.lookPitch = Math.max(-1, Math.min(1, this.lookPitch));
        });
    ...
    clearTiles() {
        for (let k in this.tiles) {
            this.tileGroup.remove(this.tiles[k].mesh);
            if (this.tiles[k].mesh.material.map) this.tiles[k].mesh.material.map.dispose();
            this.tiles[k].mesh.material.dispose();
            this.tiles[k].mesh.geometry.dispose();
        }
        this.tiles = {};
        this._loadQueue = [];
    }
    ...
    setWeatherVfx(v) {
        this.weatherVfxEnabled = v;
        if (!v) {
            this.vfxCtx.clearRect(0, 0, this.vfxCanvas.width, this.vfxCanvas.height);
            this.weatherMat3D.opacity = 0;
        }
    }

    latLonToMercator(lat, lon) {
        return {
            x: R_EARTH_M * lon * DEG_TO_RAD,
            y: R_EARTH_M * Math.log(Math.tan(Math.PI / 4 + lat * DEG_TO_RAD / 2))
        };
    }
    ...
    getTileCoords(lat, lon, zoom) {
        ...
        return {
            ...
            ty: Math.floor(n * (1 - (Math.log(Math.tan(r) + 1 / Math.cos(r)) / Math.PI)) / 2)
        };
    }
    ...
    _processQueue() {
        ...
        this.textureLoader.load(task.url, (tex) => {
            ...
            task.mat.map = tex;
            task.mat.color.set(0xffffff);
            task.mat.needsUpdate = true;
            ...
            if (this.terrainEnabled) {
                ...
                img.onload = () => {
                    ...
                    for (let i = 0; i < pos.count; i++) {
                        ...
                        const col = i % 33;
                        const px = Math.floor((col / 32) * 255);
                        const py = Math.floor((row / 32) * 255);
    ...
    updateEnvironment(lat, lon) {
        ...
        if (window.Astronomy) {
            ...
            const sunAlt = sunHor.altitude * DEG_TO_RAD;
            const sunAz = (-sunHor.azimuth + 180) * DEG_TO_RAD;
            const sd = 160000;
            ...
            const mAlt = mHor.altitude * DEG_TO_RAD;
            const mAz = (-mHor.azimuth + 180) * DEG_TO_RAD;
            const md = 100000;
            ...
            let cloudFactor = 0.0;
            if (this.currentWeatherCode === 1 || this.currentWeatherCode === 2) cloudFactor = 0.3;
            else if (this.currentWeatherCode === 3) cloudFactor = 0.6;
            else if (this.currentWeatherCode >= 45) cloudFactor = 1.0;
            ...
            if (t >= 0) {
                ...
                sInt = 1.0 - t;
                this.cloudMat.opacity = Math.min(1.0, cloudFactor * t);
            ...
            if (isNight && this.hudEnabled) {
                ...
                fogC = new THREE.Color(0x041a04);
                botC = fogC.clone();
                topC = new THREE.Color(0x020c02);
                midC = new THREE.Color(0x031203);
                ...
                this.ambientLight.intensity = isSch ? 0.15 : 0.4;
            ...
            this.skyUniforms.topColor.value.copy(topC);
            this.skyUniforms.midColor.value.copy(midC);
            this.skyUniforms.bottomColor.value.copy(botC);
    ...
    _updateWeatherVFX(speed) {
        ...
        for (let i = 0; i < this.weatherParticles.length; i++) {
            ...
            if (isSnow) {
                this.vfxCtx.arc(p.x, p.y, p.s, 0, Math.PI * 2);
            } else {
                this.vfxCtx.ellipse(p.x, p.y, p.s * 0.5, p.s * 2, p.vx * 0.1, 0, Math.PI * 2);
            }
            this.vfxCtx.fill();
            p.y += p.vy;
            p.x += p.vx;
    ...
    updateData(lat, lon, alt, head, vel, vrate) {
        ...
        for (let ix = -this.gridRange; ix <= this.gridRange; ix++) {
            for (let iy = -this.gridRange; iy <= this.gridRange; iy++) {
                const cx = tx + ix;
                const cy = ty + iy;
                const key = `${this.mapZoom}_${this.mapType}_${cx}_${cy}_${this.terrainEnabled}`;
        ...
        if (this.scene.fog.density < 0.00002) {
        ...
        const pR = Math.atan2(vrate, vel / 3.6);
        const pitch = THREE.MathUtils.lerp(0, pR, 0.08);
        ...
        this.camera.rotation.set(
            (isR ? -pitch : pitch) - 0.12 + this.lookPitch,
            pY + this.lookYaw,
            isR ? -this.currentRoll : this.currentRoll
        );
    ...
    resize() {
        ...
        this.flareCanvas.width = w;
        this.flareCanvas.height = h;
    }
    ...
    start() {
        ...
        this.smoothAlt = null;
        this.lastHdg = null;
        if (this.engineSoundEnabled) this.setEngineSound(true);
        this.resize();
    }
    ...
    stop() {
        this.isActive = false;
        if (this.audioCtx && this.engineMasterGain) {
            this.engineMasterGain.gain.setTargetAtTime(0, this.audioCtx.currentTime, 0.2);
        }
        if (this.flareCtx) {
            this.flareCtx.clearRect(0, 0, this.flareCanvas.width, this.flareCanvas.height);
        }
    }
}